...
//
  ...
  var setupOauth = function(app) {
    ...
    app.get(config['callback-url'], function(req, res) {
      ...
      passport.authenticate(config['auth-mode'], {scope: neededScope})(req, res, function() {
  ...
  var neededScope = ['user:info', 'user:check-access'];
  if (config['needed-scope']) {
    neededScope.push(config['needed-scope']);
  }
  var ensureAuthenticated = function(req, res, next) {
    ...
    switch(config['auth-mode']) {
      case 'oauth2':
        passport.authenticate(['bearer', 'oauth2'], {session: useSession, scope: neededScope})(req, res, next);
	break;
      case 'bearer':
