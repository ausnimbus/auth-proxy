...
var config = require('yargs')
  ...
  .options({
    'listen-port': {
      ...
      default: Number(process.env.OAP_SERVER_PORT || 8080)
    }, 'server-cert': {
      ...
      describe: 'Comma separated string of virtual hosts, must match number of backends',
      default: process.env.OAP_VIRTUAL_HOSTS
    }, 'backend': {
      ...
      default: process.env.OAP_CLIENT_CERT_FILE
    }, 'client-cert': {
      ...
      default: process.env.OAP_CLIENT_KEY_FILE
    }, 'auth-mode': {
      ...
      default: process.env.OAP_SERVER_CA_FILE
    }, 'session-secret': {
      describe: 'File containing secret for encrypted session cookies under "oauth2" method',
      default: process.env.OAP_SESSION_SECRET_FILE || '/var/run/secrets/kubernetes.io/serviceaccount/token'
    }, 'session-duration': {
      ...
      default: process.env.OAP_OAUTH_SECRET_FILE || '/var/run/secrets/kubernetes.io/serviceaccount/token'
    }, 'public-master-url': {
      describe: 'Public master address for redirecting clients to',
      default: process.env.OAP_PUBLIC_MASTER_URL || 'https://api.ausnimbus.com.au/'
    }, 'master-url': {
      describe: 'Internal master address proxy will authenticate against for oauth/bearer',
      default: process.env.OAP_MASTER_URL || 'https://kubernetes.default.svc.cluster.local'
    }, 'master-ca': {
      describe: 'CA certificate(s) file to validate connection to the master',
      default: process.env.OAP_MASTER_CA_FILE || '/var/run/secrets/kubernetes.io/serviceaccount/ca.crt'
    }, 'transform': {
      ...
      describe: 'Additional scope that the user requires',
      default: process.env.OAP_NEEDED_SCOPE
    }, 'check-namespace': {
      describe: 'Check namespace access instead of user',
      default: process.env.OAP_CHECK_NAMESPACE
    },
  })
  .check(function(args) {
    // yargs#demand doesn't complain on empty env var default; check for missing/invalid args here
    var errors = [];
    ['backend', 'virtualhosts', 'server-cert', 'server-key', 'server-tlsopts-file'].forEach( function(val) {
...
if (config['check-namespace']) {
  config.openshiftUserUrl = urljoin(config['master-url'], '/api/v1/namespaces/' + config['check-namespace']);
} else {
...
config.backend = typeof(config.backend) === 'string' ? config.backend.split(',') : config.backend;

if(config.virtualhosts) {
    config.virtualhosts = typeof(config.virtualhosts) === 'string' ? config.virtualhosts.split(',') : config.virtualhosts;
} else {
  config.virtualhosts = ['/']; //if only one backend is specified without virtualhost then use it for base path
}
